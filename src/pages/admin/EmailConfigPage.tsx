...
import { Table, Button, Modal, Form, Input, InputNumber, Switch, Space, Typography, message, Popconfirm, Tag } from 'antd';
...
const EmailConfigPage: React.FC = () => {
  ...
  const columns: ColumnsType<EmailConfig> = [
    ...
    { title: 'IMAP主机', dataIndex: 'imap_host', key: 'imap_host', width: 180 },
    { title: '端口', dataIndex: 'imap_port', key: 'imap_port', width: 80 },
    { title: '检查间隔(分)', dataIndex: 'check_interval', key: 'check_interval', width: 100 },
    { title: '状态', dataIndex: 'is_active', key: 'is_active', width: 80, render: (v: boolean) => v ? <Tag color="green">启用</Tag> : <Tag>停用</Tag> },
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Modal title={editing ? '编辑邮箱配置' : '新增邮箱配置'} open={modalOpen} onOk={handleSave} onCancel={() => setModalOpen(false)} destroyOnClose width={520}>
        <Form form={form} layout="vertical">
          <Form.Item name="email_address" label="邮箱地址" rules={[{ required: true, type: 'email' }]}><Input /></Form.Item>
          <Form.Item name="imap_host" label="IMAP主机" rules={[{ required: true }]} initialValue="imap.qiye.aliyun.com"><Input /></Form.Item>
          ...
          <Form.Item name="check_interval" label="检查间隔(分钟)" rules={[{ required: true }]} initialValue={5}><InputNumber min={1} max={60} style={{ width: '100%' }} /></Form.Item>
