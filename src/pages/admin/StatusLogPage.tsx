...
import { statusLogApi } from '../../services/api';
...
const StatusLogPage: React.FC = () => {
  ...
  const loadLogs = async () => {
    setLoading(true);
    try { const res = await statusLogApi.list(); setLogs(res.data); } catch { /* ignore */ }
    finally { setLoading(false); }
  };

  const columns: ColumnsType<StatusLog> = [
    ...
    { title: '操作人', key: 'operator', width: 120, render: (_: unknown, r: StatusLog) => r.operator?.username || '-' },
