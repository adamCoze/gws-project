...
const StatusLogPage: React.FC = () => {
  ...
  const columns: ColumnsType<StatusChangeLog> = [
    ...
    { title: '工作项标题', dataIndex: 'work_item_title', key: 'work_item_title', width: 200, ellipsis: true, render: (v: string) => v || '-' },
    { title: '原状态', dataIndex: 'old_status', key: 'old_status', width: 100, render: (s: string) => <Tag color={STATUS_COLORS[s as WorkItemStatus]}>{STATUS_LABELS[s as WorkItemStatus] || s}</Tag> },
    { title: '新状态', dataIndex: 'new_status', key: 'new_status', width: 100, render: (s: string) => <Tag color={STATUS_COLORS[s as WorkItemStatus]}>{STATUS_LABELS[s as WorkItemStatus] || s}</Tag> },
