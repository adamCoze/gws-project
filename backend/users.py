...
def _validate_role_fields(role_level: int, department_id, district_id, role_desc="该角色"):
    """根据角色等级校验部门/区域字段的必填规则
    ...
    if role_level == RoleLevel.DEPT_DIRECTOR:  # 5
        if not department_id:
            raise HTTPException(status_code=400, detail=f"{role_desc}（部门总监）必须选择部门")
    elif role_level == RoleLevel.DISTRICT_MANAGER:  # 4
        if not district_id:
            raise HTTPException(status_code=400, detail=f"{role_desc}（区域总监）必须选择区域")
    else:  # 3, 2, 1
        if not department_id:
            raise HTTPException(status_code=400, detail=f"{role_desc}必须选择部门")


_SECONDARY_ROLE_LABELS = {
    1: "顾问", 2: "专员", 3: "经理",
    4: "区域总监", 5: "部门总监",
    6: "监察主任", 7: "集团总监",
    8: "总裁", 9: "管理员",
...
def _normalize_secondary_roles(secondary_roles, role_level: int) -> list:
    """规范化副角色列表（对象数组）：去重、排除主角色、校验部门/区域、排序

    格式：[{"role_level": 3, "department_id": 5, "district_id": 2}, ...]
    兼容旧格式（整数数组），自动包装为对象数组。
    """
    ...
    normalized = []
    seen = set()
    for item in secondary_roles:
        if isinstance(item, (int, float)):
            item = {"role_level": int(item), "department_id": None, "district_id": None}
        elif isinstance(item, dict):
            item = {
                "role_level": int(item.get("role_level")),
        ...
        # 校验部门/区域必填
        label = _SECONDARY_ROLE_LABELS.get(rl, f"等级{rl}")
    ...
    # 按 role_level 降序排列（高级在前）
    normalized.sort(key=lambda x: x["role_level"], reverse=True)
    return normalized
